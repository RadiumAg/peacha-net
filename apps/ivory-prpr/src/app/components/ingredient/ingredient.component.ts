...
export class IngredientComponent
    ...
    set Process(value: number) {
        ...
        if (!Number.isNaN(value)) {
            const width =
                ((this.ngredientBody.nativeElement as HTMLElement).clientWidth -
                    (this.dotHead.nativeElement as HTMLElement).offsetWidth) *
                (Number(value) / 100);
    ...
    private init() {
        if (!this.ngredientBody) {
            this.ngredientBody = new ElementRef<any>(
                document.getElementById('ingredient-body')
            );
        }
        if (!this.dotHead) {
            this.dotHead = new ElementRef<any>(
                document.getElementById('dothead')
            );
        }
        if (!this.ingredientProcess) {
            this.ingredientProcess = new ElementRef<any>(
                document.getElementById('ingredientprocess')
            );
        }
    }
    ...
    start(e: MouseEvent) {
        ...
        this.setMouser(e);
        if (!this.isBind) {
            this.sliderWidth = (this.ngredientBody
                .nativeElement as HTMLElement).clientWidth;
            this.isBind = true;
            document.addEventListener('mousemove', this.mouseMove.bind(this));
            document.addEventListener('mouseup', this.mouseUp.bind(this));
            (this.dotHead.nativeElement as HTMLElement).addEventListener(
                'mouseup',
    ...
    private setMouser(e: MouseEvent) {
        ...
        this.mouser.min = 0;
        this.mouser.start =
            e.pageX - (this.dotHead.nativeElement as HTMLElement).offsetLeft;
    }
    ...
    private setProcess(left: number) {
        this.re2.setStyle(
            this.ingredientProcess.nativeElement,
            'width',
            left + 'px'
        );
    }
