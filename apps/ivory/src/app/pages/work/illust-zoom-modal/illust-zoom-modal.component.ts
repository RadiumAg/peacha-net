...
export class IllustZoomModalComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly getOldLeftAndTop = {
    ...
    getPrecent: {
      getLeft: (target: HTMLElement): number => {
        ...
        return (
          (Number(target.style.left.replace('%', '')) / 100) * el.clientWidth
        );
      },
      getTop: (target: HTMLElement): number => {
        const el = this.el.nativeElement as HTMLElement;
        return (
          (Number(target.style.top.replace('%', '')) / 100) * el.clientHeight
        );
      },
    },
  ...
  scaleImg(config: {
    ...
    if (config.scale) {
      if (oldScale <= config.max - 0.1) {
        config.target.style.transform =
          config.target.style.transform.replace(config.regex, '') +
          `scale(${oldScale + config.step + ''})`;
      } else {
        config.target.style.transform =
          config.target.style.transform.replace(config.regex, '') +
          `scale(${config.max})`;
  ...
  draggle = (ev: MouseEvent) => {
    (this.img.nativeElement as HTMLElement).style.left =
      this.mouse.start.oldLeft + (ev.pageX - this.mouse.start.x) + 'px';
    (this.img.nativeElement as HTMLElement).style.top =
      this.mouse.start.oldTop + (ev.pageY - this.mouse.start.y) + 'px';
  };
