...
export class N7rGoodDetail implements OnDestroy {
	...
	constructor(
		...
		@Inject(MODAL_DATA_TOKEN) public date: { type: number; good: Good },
		private http: HttpClient,
		private fb: FormBuilder,
		private modal: ModalService,
		private router: Router
	) {
		this.indexGood = date.good;
	...
	goto(): void {
		this.isLogin$.subscribe(is => {
			if (is) {
				if (1618632000000 <= new Date().getTime()) {
					if (this.btnType$.value === 1) {
						if (this.infoForm.valid) {
							this.http
								.post<{ orderId: number }>(`/advance/create_order`, {
									g: this.indexGood.id,
									nu: this.count$.value,
					...
					this.modal.open(PopTips, ['2021年4月17日 12:00 开始预售', false]);
				}
			} else {
				this.modalRef.close();
	...
	selectAddress(el: ElementRef): void {
		this.modal
			.openFloat(AddressSelect, el, null, true)
			.afterClosed()
			.subscribe((s: any) => {
				this.indexCity = s;
					...
					.subscribe(s => {
						this.http
							.get<{
								channelId: number;
								page: string;
							}>(`/trade/pay/cashier/launch?channelId=1&payId=${s.payId}`)
							.subscribe(payResult => {
	...
	sureCancel(): void {
		this.http
			.post('/shopmall/orders/close', {
				o: [this.orderId],
			})
			.subscribe(
				s => {
					this.modal
						.open(PopTips, ['取消订单成功', false, 1])
						.afterClosed()
						.subscribe(_ => {
							this.modalRef.close();
						});
