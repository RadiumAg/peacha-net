...
export class PurchaseResultsPage implements OnInit, OnDestroy {
	constructor(private route: ActivatedRoute, private router: Router, private http: HttpClient, private modal: ModalService) {}
	...
	ngOnInit(): void {
		combineLatest([this.route.queryParams, timer(1000, 2000)])
			.pipe(
				take(30),
				switchMap(([params, i]) => {
					if (i == 29) {
					...
					return this.http.post<OrderState>('/mall/order_state', {
