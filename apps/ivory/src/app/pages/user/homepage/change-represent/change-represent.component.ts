...
export class ChangeRepresentComponent {
  ...
  work$ = this.http
    .get<Works>(
      `/work/get_works?u=${this.list[0]._value.id}&p=0&s=12&ws=-1&k=&c=-1`
    )
    .pipe(
      take(1),
      tap((s) => {
  ...
  search(keyword: HTMLInputElement) {
    ...
    if (this.is) {
      ...
      this.http
        .get<Works>(
          `/work/get_works?u=${this.list[0]._value.id}&p=0&k=${keyword.value}&s=12&ws=-1&c=-1`
        )
        .pipe(
          take(1),
          tap((s) => {
  ...
  @HostListener('scroll', ['$event']) public scrolled($event: Event) {
    if (
      this.box.nativeElement.scrollHeight - this.box.nativeElement.scrollTop ==
      this.ne.nativeElement.clientHeight - 25
    ) {
        ...
        .pipe(
          ...
          switchMap(([p, k, count]) => {
            if (count / 12 > p) {
              return this.http
                .get<Works>(
                  `/work/get_works?u=${this.list[0]._value.id}&p=${p}&k=${k}&s=12&ws=-1&c=-1`
                )
                .pipe(
                  tap((s) => {
                    s.list.map((a) => {
  ...
  @HostListener('scroll', ['$event']) public scrolledOne($event: Event) {
    if (
      this.boxone.nativeElement.scrollHeight -
        this.boxone.nativeElement.scrollTop ==
      this.neone.nativeElement.clientHeight - 25
    ) {
        ...
        .pipe(
          ...
          switchMap(([count, p]) => {
            if (count / 12 > p) {
              return this.http
                .get<Works>(
                  `/work/get_works?u=${this.list[0]._value.id}&p=${p}&s=12&k=&ws=-1&c=-1`
                )
                .pipe(
                  tap((s) => {
                    s.list.map((a) => {
