							...
							tap(s => {
								...
								if (p === 0) {
									...
									this.hellobox.nativeElement.scrollTop =
										this.hellobox.nativeElement.scrollHeight - this.box.nativeElement.clientHeight;
								}
...
function select(
	...
	if (new Date(dayCompare$.value).getDate() == new Date(l.speaktime).getDate()) {
		const hour = Math.abs(new Date(dayCompare$.value).getHours() - new Date(l.speaktime).getHours());
		const minutes = Math.abs(new Date(dayCompare$.value).getMinutes() - new Date(l.speaktime).getMinutes());
		if (hour == 0) {
			if (minutes > 10) {
				l.showtime = true;
