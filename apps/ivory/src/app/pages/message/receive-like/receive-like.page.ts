...
export class ReceiveLikePage implements OnDestroy {
  ...
  receiveLike$ = this.page$
    .pipe(
      ...
      switchMap(([p, c]) => {
        if (c === 10 || p === 0) {
          return this.http
            .get<{ list: Detail[] }>(`/news/star?page=${p}&size=10`)
            .pipe(
              tap((s) => {
  ...
  toDetail(url: string, id: string): void {
    this.http
      .get<{ rootid: number; root_index: number; sub_index: number }>(
        `/comment/reply_jump?c=${id}`
      )
      .subscribe((s) => {
        if (s.root_index != -1) {
          window.open(
            `${url}?root=${s.root_index}&sub=${s.sub_index}&rootid=${
              s.rootid
            }#${'reply' + id}`
          );
        } else {
