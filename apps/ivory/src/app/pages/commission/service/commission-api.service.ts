...
export class CommissionApiService {
   ...
   * @version
   */
  public uploadFile = (f: FormData) =>
    this.http.post<{ token: string; url: string }>('/common/upload_file', f);
   ...
   * @version 2020/12/4
   */
  public commissionStatus = (c: number) =>
    this.http.get<{ status: number }>(`/commission/commission_status?c=${c}`);
   ...
   * @version 2020/11/6
   */
  public detail = (c: number) =>
    this.http.get<CommissionDetail>(`/commission/detail?c=${c}`);
  ...
  public homePage = (
    ...
    this.http.get<{
      ...
      `/commission/home_page?c=${c}&p=${p}&s=${s}&k=${k ?? ''}${
        mip ? '&mip=' + mip : ''
      }${map ? '&map=' + map : ''}`
    );
  ...
  public createOrder = (c: number) =>
    this.http.post<{ pageRoute: string; payId: number }>(
      '/commission/create_order',
      {
        c,
      }
    );
   ...
   * @version 2020/12/11
   */
  public nodeModifyCount = (c: number) =>
    this.http.get<{ count: number }>(`/commission/node/modify_count?c=${c}`);
  ...
  public auditTips = (t: number, s: number) =>
    this.http.post<{ payId: number; pageRoute: number }>(
      `/commission/audit_tips`,
      {
        t,
        s,
      }
