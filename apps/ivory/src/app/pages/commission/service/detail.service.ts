...
export class CommissionDetailService {
  ...
  isSponsorTimeout() {
    if (this.day > 10) {
      if (
        new Date().getTime() - this.detail.commission.startTime >
        5 * 24 * 60 * 60 * 1000
      ) {
        return true;
      } else {
  ...
  getCommissionLasttime() {
    return (
      Number(this.detail.commission.startTime) +
      Number(this.day) * 24 * 60 * 60 * 1000
    );
  }
  ...
  getIdentity() {
    if (
      this.id === this.detail?.receiver.id &&
      this.id != this.detail?.sponsor.id
    ) {
      return 2;
    } else if (
      this.id != this.detail.receiver.id &&
