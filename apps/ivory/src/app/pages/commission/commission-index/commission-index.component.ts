	...
	list$: Observable<{
		...
		switchMap(params => {
			...
			switch (Number(this.sw)) {
				case 0: {
					return this.commissionApi.homePage(params.c ?? -1, this.currentPage - 1, 4, params.k, params.min, params.max);
				}
				case 1: {
					if (!params.t) {
						this.router.navigate([], {
					...
					return this.commissionApi.launchList(params.t, this.currentPage - 1, 4);
				}
				case 2: {
					if (!params.t) {
						this.router.navigate([], {
					...
					return this.commissionApi.signUpList(params.t, this.currentPage - 1, 4);
				}
			}
		})
	);
