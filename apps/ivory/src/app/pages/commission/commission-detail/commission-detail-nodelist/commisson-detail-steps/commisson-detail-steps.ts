...
export class CommissionDetailSteps implements OnInit {
	...
	ngOnInit(): void {
		this.commissionApi.nodeModifyCount(this.detail.getCommissionId()).subscribe(i => {
			if (this.detail.getDetailValue().commission.modifyCount >= i.count) {
				this.modifyCount = this.detail.getDetailValue().commission.modifyCount - i.count;
			} else {
				this.modifyCount = 0;
			}
		...
		this.commissionApi.nodeList(this.detail.getCommissionId()).subscribe(s => {
			this.dataList = s.list;
			this.cdr.detectChanges();
			this.active = this.dataList.findIndex(l => l.status != 3);
			this.detail.setCurrentNodeRate(this.dataList[this.active == 0 ? 0 : this.active - 1]?.rate);
	...
	isSponsorTimeout(): boolean {
		if (this.detail.getDetailValue().commission.day > 10) {
			if (new Date().getTime() - this.indexNodeDetail?.recordlist.list[0]?.submitTime > 5 * 24 * 60 * 60 * 1000) {
				return true;
			} else {
				return false;
			}
		} else {
			if (new Date().getTime() - this.indexNodeDetail?.recordlist.list[0]?.submitTime > 3 * 24 * 60 * 60 * 1000) {
				return true;
			} else {
				return false;
			}
	...
	// 上传图片
	updatePic(event: any, input: HTMLInputElement, u: UploadImageDirective): void {
	...
	result(e: any): void {
		if (typeof e === 'string') {
			...
			this.isShowReportProgress = false;
			this.fileList.push({
				name: '文件' + (this.fileList.length + 1) + '.' + e.url.split('.')[e.url.split('.').length - 1],
				token: e.token,
				url: e.url,
			});
			this.cdr.detectChanges();
	...
	updateFile(event: any, input: HTMLInputElement, u: UploadImageDirective): void {
	...
	confirm(p: number): void {
		this.modal
			.open(PopTips, [p === 0 ? '是否确认企划无误？' : '是否确认企划有误？', true])
			.afterClosed()
			.subscribe(is => {
				if (is) {
		...
		if (this.detail.commissionStatus$.value === 2) {
			if (p != 1) {
				if (p === 3 && this.modifyCount === 0) {
					this.modal.open(PopTips, ['退回修改次数为0，无法发起退回修改。', false]);
					...
					this.modal
						...
						.subscribe(i => {
							if (i) {
								if (i[1]) {
									this.commissionApi.nodeAudit(this.indexNode.id, p, i[0], i[1]).subscribe(
										_ => {
											// this.index_status = 4;
											this.indexNode = { ...this.indexNode, status: 4 };
											...
											if (p === 3) {
												...
												this.indexNode.rejectCount = this.indexNode.rejectCount - 1;
											}
											this.cdr.detectChanges();
										},
										e => {
									...
									this.commissionApi.nodeAudit(this.indexNode.id, p, i[0]).subscribe(
										_ => {
											// this.index_status = 4;
											this.indexNode = { ...this.indexNode, status: 4 };
											this.dataList[this.active].status = 4;
				...
				this.modal
					.open(PopTips, [this.indexNode.type === 1 ? '是否确认绑定文件无误？' : '是否确认审核通过此阶段节点？', true])
					.afterClosed()
					.subscribe(is => {
						if (is) {
									...
									_o => {
										this.commissionApi.nodeAudit(this.indexNode.id, p).subscribe(
											_ => {
												...
												this.detail.setCurrentNodeRate(this.dataList[this.active]?.rate);
	...
	edit(): void {
		this.router.navigate(
			[this.detail.getDetailValue().commission.category === 1 ? '/commission/publish/illust' : '/commission/publish/live2d'],
			{
				queryParams: {
