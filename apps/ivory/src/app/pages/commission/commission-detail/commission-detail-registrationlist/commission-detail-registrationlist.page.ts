...
export class CommissionDetailRegistrationlistPage implements OnInit {
	...
	rlist$ = this.route.queryParams.pipe(
		...
		switchMap(p => {
			return this.commissionApi.registrationList(p.id, p.page ? p.page - 1 : 0, p.s ?? 4).pipe(
				tap(s => {
					this.list = s;
					/**请求报名列表的作品列表 */
					if (s.count > 0) {
	...
	select(nick: string, price: number, day: number, start: number, avatar: string, userid: number): void {
		this.modal
			...
			.subscribe(i => {
				if (i) {
					this.commissionApi.selectUser(userid, this.detail.getCommissionId()).subscribe(_ => {
	...
	getWorkList(work: Array<number>): void {
		this.commissionApi.userWorks(work.join(',')).subscribe(w => {
			w.list.map(worklist => {
				const index = this.list.list.findIndex(i => i.userId == worklist.userid);
