...
export class CommissionRegistration {
	...
	constructor(private modalRef: ModalRef<CommissionRegistration>, private fb: FormBuilder) {
		this.registrationForm = this.fb.group(
			{
				...
				money: new FormControl('', [Validators.required, Validators.max(99999), Validators.pattern(/^(\d){1,}0$/)]),
	...
	dateValidator: ValidatorFn = (control: FormGroup): ValidationErrors | null => {
		...
		if (res.test(year.value) && month_res.test(month.value) && res.test(day.value) && year.value >= 2020) {
			if (
