...
export class CommissionDiscontinuePage implements OnInit {
    ...
    switchMap(([r, p]) => {
      return this.api.discontinueList(p.id).pipe(
        tap((s) => {
          if (
            s.list[0]?.status === 0 ||
            ([3, 0].includes(s.list[0]?.type) && s.list[0]?.status === 1)
          ) {
            this.latestRecord = {
              id: s.list[0]?.id,
  ...
  submitFile(): void {
    ...
    if (this.fileList.length > 0) {
      this.api
        .discontinueSubmitFile(this.detail.getCommissionId(), list)
        .subscribe(
          (_) => {
            this.refresh$.next(3);
          },
