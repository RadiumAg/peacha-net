...
export class CommissionDetailComponent implements OnInit, OnDestroy {
  ...
  select(): void {
    this.showBtn.map((l) => {
      ...
      if (l.a === 3) {
        if (
          Number(this.commissionStatus$.value) === 1 ||
          Number(this.commissionStatus$.value) === 0
        ) {
          return (l.word = [l.word[1]]);
        } else {
          return (l.word = [l.word[0]]);
  ...
  ngOnInit(): void {
    ...
    this.commissionApi
      .registrationList(this.detailState.getCommissionId(), 0)
      .subscribe((s) => {
      ...
      if (status != 5) {
        this.id$.subscribe((id) => {
          if (this.detail.sponsor.id === id) {
            if (
              status === 7 &&
              this.detail.commission.expireTime < this.nowTime
            ) {
              this.btnIconList = [
                {
                  text: '编辑企划',
  ...
  remainingTime(t: number): void {
    ...
    const m = Math.floor(
      (time - day * 1000 * 60 * 60 * 24 - h * 1000 * 60 * 60) / 1000 / 60
    );
  ...
  stopCommission(): void {
    this.model
      ...
      .subscribe((i) => {
        if (i) {
          this.commissionApi
            .setStatus(this.detailState.getCommissionId(), 7)
            .subscribe(
              (_) => {
                this.commissionApi
                  .commissionStatus(this.detailState.getCommissionId())
                  .subscribe((s) => {
  ...
  startCommission(): void {
    this.commissionApi
      .setStatus(this.detailState.getCommissionId(), 0)
      .subscribe(
        (_) => {
          this.commissionApi
            .commissionStatus(this.detailState.getCommissionId())
  ...
  closeCommission(): void {
    this.model
      ...
      .subscribe((i) => {
        if (i) {
          this.commissionApi
            .setStatus(this.detailState.getCommissionId(), 5)
            .subscribe(
              (_) => {
                this.commissionApi
                  .commissionStatus(this.detailState.getCommissionId())
        ...
        if (i) {
          this.commissionApi
            .signUp(
              ...
              i[1].time,
              new Date(
                i[1].start_year + '/' + i[1].start_month + '/' + i[1].start_day
              ).getTime(),
              i[1].money / 10
            )
            .subscribe(
              ...
              (e) => {
                if (e.code === 404) {
                  ...
                  this.model.open(PopTips, [
                    '您还未完成画师/模型师认证，无法报名此企划，请前往认证！',
                    false,
                  ]);
                }
              }
  ...
  canceRegistration(): void {
    this.model
      ...
      .subscribe((i) => {
        if (i) {
          this.commissionApi
            .cancel(this.detailState.getCommissionId())
            .subscribe(
              (_) => {
    ...
    if ([1, 2, 4].includes(this.detailState.commissionStatus$.value)) {
      this.model
        ...
        .subscribe((i) => {
          if (i) {
            this.commissionApi
              .delayTime(this.detailState.getCommissionId(), i)
              .subscribe(
                (s) => {
                  this.detailState.setCommissionDay(
                    this.detailState.getCommissionDay() + i
                  );
                  this.commissionDay = this.commissionDay + Number(i);
                  this.lasttime =
                    this.lasttime + Number(i) * 24 * 60 * 60 * 1000;
                  this.cdr.detectChanges();
                },
                (e) => {
                  this.isError.ifError(e.code);
  ...
  edit(): void {
    this.router.navigate(
      [
        this.detailState.getDetailValue().commission.category === 0
  ...
  discontinue(): void {
    if (
      Number(this.detail.commission.status) === 1 ||
      Number(this.detail.commission.status) === 0
    ) {
  ...
  func(i: number): void {
    if (
      !(
        (this.commissionStatus$.value === 6 ||
          this.commissionStatus$.value === 4) &&
        i === 1
      )
    ) {
      switch (i) {
