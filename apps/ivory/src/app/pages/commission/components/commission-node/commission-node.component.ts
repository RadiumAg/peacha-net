...
export class CommissionNodeComponent
  implements OnInit, ControlValueAccessor, AfterViewInit, AfterViewChecked {
  ...
  maxRate = 40;
  isCanDelete = false;
  nodes$: BehaviorSubject<
    Partial<{ n: string; r: number; symbol: symbol }>[]
  > = new BehaviorSubject([]);
  nodesSpecial$: BehaviorSubject<
    Partial<{ n: string; r: number; symbol: symbol }>[]
  > = new BehaviorSubject([]);
  isHasError = true;
  ...
  nodesDefalut: Partial<{ n: string; r: number; symbol: symbol }>[] = [
    { r: 100, n: '提交源文件' },
  ];
  @ViewChild('cut')
  ...
  verify = {
    maxSizeValidator: () => {
      if (
        this.nodes$.getValue().concat(this.nodesSpecial$.getValue()).length >=
        this.maxSize
      ) {
        return false;
      }
  ...
  private setSpecial(isCute: boolean) {
    ...
    if (isCute) {
      ...
      this.nodesSpecial$.next([
        ...this.nodesSpecial$.getValue().filter((x) => x.n != '立绘拆分'),
      ]);
    }
  }
  ...
  addNode() {
    ...
    this.nodes$.next([
      ...this.nodes$.getValue(),
      { symbol: Symbol(), n: '新建节点', r: 10 },
    ]);
  }

  endEdit(e: Event, symbol: symbol, typeId: 0 | 1 | 2 = 0) {
    const target = e.target as HTMLInputElement;
    const targetIndex = this.nodes$
      .getValue()
      .findIndex((x) => x.symbol == symbol);
    if (typeId === 0) {
    ...
    if (oldValue[targetIndex]?.n.trim() == '') {
      this.nodes$.next(
        oldValue.filter((value, index) => index !== targetIndex)
      );
    }
  }

  private toggleHidden(
    showTarget: ElementRef<HTMLElement>,
    hiddenTarget: HTMLElement
  ) {
  ...
  private setErrorMessage(result: Set<number>) {
    ...
    this.commissionErrors?.forEach((item, index, arr) => {
      if (result.has(index)) {
        ...
        this.re2.setProperty(
          item.nativeElement,
          'innerText',
          '每个阶段节点间的稿酬比例最低不得低于10%，最大不得超过40%'
        );
  ...
  private compare(
    ...
    if (
      prelowValue > this.maxRate ||
      prelowValue < this.minRate ||
      nextlowValue > this.maxRate ||
      nextlowValue < this.minRate
    ) {
      result.add(index);
    }
  }
  ...
  deleteNode(symbol: symbol) {
    if (
      this.nodes$.getValue().length + this.nodesSpecial$.getValue().length <=
      this.minSize
    ) {
      return;
    }
