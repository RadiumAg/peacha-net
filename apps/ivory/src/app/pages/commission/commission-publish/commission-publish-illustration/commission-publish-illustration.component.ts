...
export class CommissionPublishIllustrationComponent
  ...
  private setPage(event: Event, currentIndex: number): number {
    const flag = Number(
      (event.target as HTMLElement).getAttribute('data-flag')
    );
    if (flag === 1) {
      if (currentIndex >= 1) {
    ...
    this.detailInfoForm.valueChanges
      .pipe(
        ...
        map((_) => {
          ...
          result['wi'] = _.wi;
          result['f'] =
            _.f.length != 0 ? _.f.map((x) => x.token || x.url)[0] : '';
    ...
    this.nodeForm.valueChanges
      .pipe(
        ...
        map((_) => {
          ...
          result['spr'] =
            _.fl.filter((x) => x.n == '立绘拆分').length != 0
              ? _.fl.filter((x) => x.n == '立绘拆分')[0].r
              : 0;
          return result;
        })
      )
