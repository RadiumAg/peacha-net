...
export class RegisterPage implements OnInit {
	checkForm: FormGroup;
	constructor(private fb: FormBuilder, private store: Store, private router: Router, private cdr: ChangeDetectorRef) {
		this.checkForm = this.fb.group({
			method: new FormControl('phone', []),
			account: new FormControl('', [Validators.required, this.confirmValidator]),
	...
	submit() {
		if (this.checkForm.valid) {
			this.store
				.dispatch(
					new Register(
						this.checkForm.value.method === 'phone' ? this.checkForm.value.account + ',86' : this.checkForm.value.account,
