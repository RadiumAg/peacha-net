...
export class SearchPage implements OnInit, OnDestroy {
  ...
  label$ = this.route.queryParams.pipe(
    switchMap((s) => {
      this.order.setValue(
        s.keyword === undefined ? 1 : s.o === undefined ? '0' : s.o,
        { emitEvent: false }
      );
      this.time.setValue(s.dd === undefined ? '0' : s.dd, { emitEvent: false });
      this.userOrder.setValue(
        s.keyword === undefined ? 1 : s.o === undefined ? '0' : s.o,
        { emitEvent: false }
      );
      ...
      return this.http.get<any>(
        `/work/tag_count?t=${this.keyword$.value ?? ''}`
      );
    })
  );
  searchType$ = concat(
  ...
  ngOnInit(): void {
    this.orderSubscription = this.order.valueChanges
      .pipe()
      .subscribe((o) => this.updateParams({ o }));
    this.timeSubscription = this.time.valueChanges
      .pipe()
      .subscribe((dd) => this.updateParams({ dd }));
    this.userOrderSubscription = this.userOrder.valueChanges
      .pipe()
      .subscribe((o) => this.updateParams({ o }));
    this.userTypeSubscription = this.userType.valueChanges
      .pipe()
      .subscribe((r) => this.updateParams({ r }));
  }
