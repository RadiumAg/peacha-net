...
export class OrderListPage {
	...
	order$: Observable<OrderInfo> = combineLatest([this.route.queryParams, this.update$]).pipe(
		switchMap(([r]) => {
			return this.http.get<any>(`/mall/get_buy_order?k=${r.k ?? ''}&t=${r.t ?? -1}&p=${r.p ? r.p - 1 : 0}&s=4`).pipe(
				tap(s => {
