...
export class SelectCompanyComponent implements ControlValueAccessor, OnInit {
  isHaveCompany$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
    ...
    this.uid$
      .pipe(
        tap((x) => {
          ...
          this.http
            .get(`/user/get_user`, { params })
            .subscribe((r) => this.user$.next(r));
          this.uid = x;
        })
      )
      .subscribe();
  ...
  writeValue(obj: any): void {
    ...
    if (obj.cooperateid) {
      ...
      this.http
        .get('/work/get_cooperate_detail', { params })
        .subscribe((x: CooperateDetail) => {
  ...
  addCompany() {
    this.modal
      .open<CompanyModalComponent<UserStateModel>, object>(
        CompanyModalComponent,
        {
          ...
          title: '选择合作者',
          tips:
            'TIPS：为保障您与合作者的利益，避免不必要的纠纷，请您详细商定分成事宜后再发起合作',
          imgprototype: 'avatar',
