...
import {
  ...
  validator,
} from 'libs/peacha-core/src/lib/core/commom/common';
...
export class Live2dFreeComponent implements OnInit, AfterViewInit {
  ...
  private setMainForm(r: Work): void {
    ...
    this.live2dUpload.loadFileFromOpal(
      r.file,
      r.file_data ? JSON.parse(r.file_data) : null
    );
  }
  ...
  private setModelChecked(r: Work): void {
    if (JSON.parse(r.file_data)) {
      this.checkedForm.patchValue({
        enableFaceTrackerChecked: JSON.parse(r.file_data).enableFaceTracker
          ? true
          : false,
        enableSettingPanelChecked: JSON.parse(r.file_data).enableSettingPanel
          ? true
          : false,
        freeModelDownLoadChecked:
          r.goods_list.length && r.goods_list[0].sell_state > 0 ? true : false,
      });
    }
  }
  ...
  private getCopyRight(): void {
    ...
    this.api
      .copyright(0)
      .subscribe((x: { list: { name: string; id: number }[] }) => {
