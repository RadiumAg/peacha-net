...
@Component({
    selector: 'ivo-3d-fail',
    templateUrl: './3d-fail.page.html',
    styleUrls: ['./3d-fail.page.less'],
})
export class ThreeDFailPage {
    ...
    works$ = combineLatest([this.update$, this.route.queryParams]).pipe(
        switchMap(([_up, params]) => {
            return this.memberApi.getApplyWork(params.k, params.p, 6, 2, 2)
                .pipe(
                    tap(s => {
