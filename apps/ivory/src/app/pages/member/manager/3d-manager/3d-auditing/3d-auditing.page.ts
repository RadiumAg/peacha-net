...
@Component({
    selector: 'ivo-3d-auditing',
    templateUrl: './3d-auditing.page.html',
    styleUrls: ['./3d-auditing.page.less'],
})
export class ThreeDAuditingPage {
    showList = [];
    ...
    works$ = combineLatest([this.route.queryParams, this.update$]).pipe(
        switchMap(([params, _i]) => {
            return this.memberApi.getApplyWork(params.k, params.p, 6, 2, 0)
                .pipe(
                    tap(s => {
