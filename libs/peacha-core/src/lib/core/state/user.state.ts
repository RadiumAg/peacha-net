...
export class UserState {
    ...
    updatePublicInfo(ctx: Context, action: UpdatePublicInformation) {
        ...
        return this.http
            .post<{ avatar: string }>('/user/update_public_info', form)
            .pipe(
                tap((t) => {
                    ctx.patchState({
                        avatar:
                            action.avatar.size == undefined
                                ? ctx.getState().avatar
                                : t.avatar,
    ...
    UpdateAvatar(ctx: Context, action: UpdateAvatar) {
        ...
        form.append('a', action.avatar);
        return this.http
            .post<{ avatar: string }>('/user/update_public_avatar', form)
            .pipe(
                tap((t) => {
                    ctx.patchState({
                        avatar:
                            action.avatar == undefined ? undefined : t.avatar,
                    });
                })
            );
    }
    ...
    UpdateBanner(ctx: Context, action: UpdateBanner) {
        ...
        return this.http
            .post<{ banner: string }>('/user/update_public_banner', form)
            .pipe(
                tap((t) => {
                    ctx.patchState({
                        banner:
                            action.banner == undefined ? undefined : t.banner,
                    });
                })
            );
    }
