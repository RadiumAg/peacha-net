...
import { AddHistroy, AddOneHistroy, AddSessionOne, AddUnreadCounnt, ReduceUnreadCounnt, ReplyUnreadCounnt } from '../state/chat.action';
...
export class ChatStartService {
	...
	connect(url: string): void {
		...
		this.http.get<{ roomid: string; count: number }[]>('/chat/unread_count').subscribe(s => {
			let num = 0;
			s.forEach(l => {
				num = num + l.count;
			});
					...
					this.http
						...
						.subscribe(s => {
							if (s) {
								...
								if (this.histroy.findIndex((l: { speaktime: number }) => l.speaktime === s[0]?.speaktime) === -1) {
									let tem = [];
									tem = s.reverse();
									...
									this.store.dispatch(new AddHistroy(this.histroy)).subscribe(_ => {
										if (first) {
											this.isButtom$.next(true);
										}
									});
	 ...
	 * 发送消息
	 */
	sendMessage(id: number, value: string, type: number, me: number, url?: string): void {
		this.http
			.post<{ message: { content: string; type: number } }>('/chat/send_message', {
				r: id,
				m: value,
				t: type,
			})
			.subscribe(
				s => {
					...
					const i = this.roomList.findIndex(item => Number(item.roomid) == id);
	...
	closeRoom(id: string): void {
		...
		this.store.dispatch(new ReduceUnreadCounnt(this.roomList[i].unread)).subscribe();
			...
			.subscribe(_ => {
				id.forEach(a => {
					...
					if (this.roomList[i]?.unread) {
						this.store.dispatch(new ReduceUnreadCounnt(Number(this.roomList[i].unread))).subscribe(s => {
	...
	getList(): void {
		this.http.get<RoomList>(`/chat/room_list?r=${this.roomidList}&s=20`).subscribe(s => {
			 ...
			 * 获取未读数
			 */
			this.http.get<{ roomid: string; count: number }[]>('/chat/unread_count').subscribe(a => {
				a.forEach(l => {
					const i = this.roomList.findIndex(item => item.roomid == l.roomid);
					this.roomList[i] = {
						...this.roomList[i],
						unread: l.count,
					};
				...
				this.store.dispatch(new ReplyUnreadCounnt(this.totalUnread$.value)).subscribe();
			});
			...
			this.http
				...
				.subscribe(v => {
					v.list.forEach(l => {
						const index = this.roomList.findIndex(item => item.sender_id == l.id);
...
function compare(property: string): any {
	return function (a: { message: { speaktime: any } }, b: { message: { speaktime: any } }): any {
...
function selectShowTime(
	...
	if (new Date(Number(dayCompare$.value)).getDate() == new Date(Number(l.speaktime)).getDate()) {
		const hour = Math.abs(new Date(Number(dayCompare$.value)).getHours() - new Date(Number(l.speaktime)).getHours());
		const minutes = Math.abs(new Date(Number(dayCompare$.value)).getMinutes() - new Date(Number(l.speaktime)).getMinutes());
		if (hour == 0) {
			if (minutes > 10) {
				l.showtime = true;
