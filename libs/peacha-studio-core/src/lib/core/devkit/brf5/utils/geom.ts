export const toDegree = x => (x * 180.0) / Math.PI;
export const toRadian = x => (x * Math.PI) / 180.0;
export const length = (x, y) => Math.sqrt(x * x + y * y);
export const distance = (p0, p1) => length(p1.x - p0.x, p1.y - p0.y);
export const angle = (p0, p1) => Math.atan2(p1.y - p0.y, p1.x - p0.x);
