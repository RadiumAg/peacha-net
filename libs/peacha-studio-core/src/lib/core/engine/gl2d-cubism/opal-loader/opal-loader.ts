...
} from '../json-loader/json-cfg-pose-adapter';
...
export async function loadOpalModelFromVFS(
  ...
  if (opal.fileReferences.motions !== undefined && options?.loadMotions) {
    ...
    for (const str in opal.fileReferences.model3.FileReferences.Motions) {
      if (str !== undefined) {
        opal.fileReferences.model3.FileReferences.Motions[str].forEach(
          (motionDef) => {
  ...
  if (options?.loadExpressions) {
    ...
    if (opal.fileReferences.exp3 !== undefined) {
      opal.fileReferences.model3.FileReferences.Expressions.forEach(
        (exp3Def, index) => {
          const name = exp3Def.Name.replace('.exp3.json', '');
          const clip = loadCubismExpressionFromJson(
            opal.fileReferences.exp3[index],
            name
          );
          animator.clips.push(clip);
        }
  ...
  if ((opal.fileReferences.physics3 ?? false) && options?.loadPhysics) {
    const phi3Comp = loadPhysicsComponent(
      cubismModel,
      opal.fileReferences.physics3
    );
    ret.push(phi3Comp);
  }
  // #endregion
