import { CubismCDIJson, CubismModelJson, Input, Output, Vertex, PhyscicsDictionaty, NormalizationValue, ExpressionParameter, CubismPoseJson, CubismExpJson, CubismMotionJson, CubismPhysicsJson, CubismUserDataJson, Curve, UserData, MotionUserData } from './structures';
      ...
      if (cfgSize > -1) {
        ...
        for (let index = 0; index < cfgSize; index++) {
          ...
          if (fileNameSize > -1) {
            ...
            file = data.slice(offset, offset += fileSize);
            this.cfgs.push(file);
          }
        }
      }
        ...
        for (let index = 0; index < cubismFilesSize; index++) {
          ...
          if (fileNameSize > -1) {
            ...
            if (fileName.endsWith('.motion3.json') && !fileName.startsWith('__MACOSX')) {
              ...
              this.model3.FileReferences.Motions[Object.keys(this.model3.FileReferences.Motions)[0]]
                .push({
                  File: fileName
                });
              ...
              this.model3.FileReferences.Expressions.push({
                Name: fileName.split('/').find(str => str.endsWith('.exp3.json')).replace('.exp3.json', ''),
                File: fileName
              });
            }
          }
