...
const _csm = (function () {
	...
	return (
		...
		(_csm.initializeModelInPlace = function (moc: any,memory: any,modelSize: any): any {
		...
		(_csm.mallocMoc = function (mocSize: any): any {
			return _em.ccall('csmMallocMoc','number',['number','number'],[mocSize,o]);
		}),
		...
		(_csm.readCanvasInfo = function (model: any,outSizeInPixels: any,outOriginInPixels: any,outPixelsPerUnit: any): any {
			_em.ccall(
				'csmReadCanvasInfo',
				null,
...
export class CanvasInfo {
	...
	constructor(modelPtr: number) {
		if (modelPtr) {
			...
			const _canvasSize_nDataBytes = _canvasSize_data.length * _canvasSize_data.BYTES_PER_ELEMENT;
			const _canvasSize_dataPtr = _csm.malloc(_canvasSize_nDataBytes);
			const _canvasSize_dataHeap = new Uint8Array(_em.HEAPU8.buffer,_canvasSize_dataPtr,_canvasSize_nDataBytes);
			_canvasSize_dataHeap.set(new Uint8Array(_canvasSize_data.buffer));
			let _canvasOrigin_data = new Float32Array(2);
			const _canvasOrigin_nDataBytes = _canvasOrigin_data.length * _canvasOrigin_data.BYTES_PER_ELEMENT;
			const _canvasOrigin_dataPtr = _csm.malloc(_canvasOrigin_nDataBytes);
			const _canvasOrigin_dataHeap = new Uint8Array(_em.HEAPU8.buffer,_canvasOrigin_dataPtr,_canvasOrigin_nDataBytes);
			_canvasOrigin_dataHeap.set(new Uint8Array(_canvasOrigin_data.buffer));
			let _canvasPPU_data = new Float32Array(1);
			const _canvasPPU_nDataBytes = _canvasPPU_data.length * _canvasPPU_data.BYTES_PER_ELEMENT;
			const _canvasPPU_dataPtr = _csm.malloc(_canvasPPU_nDataBytes);
			const _canvasPPU_dataHeap = new Uint8Array(_em.HEAPU8.buffer,_canvasPPU_dataPtr,_canvasPPU_nDataBytes);
...
export class Parameters {
	...
	constructor(modelPtr: number) {
		...
		for (let _ids = new Uint32Array(_em.HEAPU32.buffer,_csm.getParameterIds(modelPtr),length),i = 0; i < _ids.length; i++) {
...
export class Parts {
	...
	constructor(modelPtr: number) {
		...
		for (let _ids = new Uint32Array(_em.HEAPU32.buffer,_csm.getPartIds(modelPtr),length),i = 0; i < _ids.length; i++) {
			this.ids[i] = _em.UTF8ToString(_ids[i]);
		}
...
export class Drawables {
	...
	constructor(modelPtr: number) {
		...
		let length = 0,
			length2 = null;
		(this.count = _csm.getDrawableCount(modelPtr)),(length = _csm.getDrawableCount(modelPtr)),(this.ids = new Array(length));
		// tslint:disable-next-line: no-var-keyword
		for (
			// eslint-disable-next-line no-var
			var _ids = new Uint32Array(_em.HEAPU32.buffer,_csm.getDrawableIds(modelPtr),length),i = 0;
			i < _ids.length;
			i++
		) {
			this.ids[i] = _em.UTF8ToString(_ids[i]);
		...
		const _masks = new Uint32Array(_em.HEAPU32.buffer,_csm.getDrawableMasks(modelPtr),length);
		...
		const _vertexPositions = new Uint32Array(_em.HEAPU32.buffer,_csm.getDrawableVertexPositions(modelPtr),length);
		...
		const _vertexUvs = new Uint32Array(_em.HEAPU32.buffer,_csm.getDrawableVertexUvs(modelPtr),length);
		...
		const _indices = new Uint32Array(_em.HEAPU32.buffer,_csm.getDrawableIndices(modelPtr),length);
...
export const Utils = (function () {
	...
	return (
		...
		(Utils.hasVertexPositionsDidChangeBit = function (bitfield: number): boolean {
