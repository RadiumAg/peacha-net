...
import { CubismPhysicsSubRig, CubismPhysicsInput, CubismPhysicsOutput, CubismPhysicsParticle } from './cubism-phyisics-defs';
...
export class CubismPhysicsSystem extends SystemBase {
	constructor() {
		super([CubismModelComponent, CubismPhysicsComponent]);
	}
	...
	update(entity: Entity, delta: number): void {
		...
		const parameterValue = model.native.parameters.values;
		const parameterMaximumValue = model.native.parameters.maximumValues;
		const parameterMinimumValue = model.native.parameters.minimumValues;
		const parameterDefaultValue = model.native.parameters.defaultValues;
...
function updateParticles(
	...
	for (let i = 1; i < strandCount; ++i) {
		strand[i].force = vec2add(vec2mulByScalar(currentGravity, strand[i].acceleration), windDirection);
