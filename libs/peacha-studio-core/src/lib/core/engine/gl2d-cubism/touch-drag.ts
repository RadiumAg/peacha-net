...
import { GL2DRenderingContext, Transform2D, Transform2DComponent, vec2sub } from '../gl2d';
...
export class TouchDragSystem extends SystemBase {
	...
	constructor(@Inject(DOM_ELEMENT) private domElement: HTMLElement, private context: GL2DRenderingContext) {
	...
	touchMove(event: TouchEvent): void {
		...
		if (event.targetTouches.length === 2) {
			this.lastScale.push({
				x: (event.targetTouches[0].clientX + event.targetTouches[1].clientX) / 2,
				y: (event.targetTouches[0].clientY + event.targetTouches[1].clientY) / 2,
				scale:
					CubismMath.distance(
						{
							x: event.targetTouches[0].clientX,
