...
export class CubismAnimationSystem extends SystemBase {
	...
	update(entity: Entity, delta: number, time: number): void {
		...
		if (queue.animationScheduled.size > 0) {
			...
			Array.from(queue.animationScheduled.values()).forEach((entry, index) => {
				if (index === queue.animationScheduled.size - 1 || entry.clip.idle) {
					entry.startTime = time;
					entry.endTime = entry.clip.duration > 0 ? time + entry.clip.duration : Number.MAX_VALUE;
					entry.fadeOutStartTime = entry.clip.duration > 0 ? time + entry.clip.duration - entry.fadeOutTime : Number.MAX_VALUE;
