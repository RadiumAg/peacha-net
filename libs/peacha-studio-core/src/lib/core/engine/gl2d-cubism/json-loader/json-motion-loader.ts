...
export function loadCubismMotionClipFromJson(
    model: CubismModel,
    def: CubismMotionJson,
    name: string,
    idle = false
): CubismAnimationClip {
    const curves = new Array(def.Meta.CurveCount)
        .fill(null)
        .map((x) => new CubismMotionCurve());
    const segments = new Array(def.Meta.TotalSegmentCount)
        .fill(null)
        .map((x) => new CubismMotionSegment());
    const points: CubismMotionPoint[] = new Array(
        def.Meta.TotalPointCount
    ).fill(null);
    ...
    for (let i = 0; i < curves.length; i++) {
        ...
        switch (curve.type) {
            ...
            default:
                throw new Error(
                    `Not a valid curve target: ${def.Curves[i].Target}`
                );
        }
        ...
        for (
            ...
            segmentPosition < def.Curves[i].Segments.length;
            ...
            if (segmentPosition == 0) {
                ...
                segments[totalSegmentCount].basePointIndex =
                    totalPointCount - 1;
            }
    ...
    const motionClip = new CubismAnimationMotion3Clip(
        def.Meta.Duration * 1000,
        def.Meta.Fps,
        curves,
        segments,
        points,
