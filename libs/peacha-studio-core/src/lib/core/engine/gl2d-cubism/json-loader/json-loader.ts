...
import {
  getCfgIdleAnimationNames,
  loadCfgClipsFromFile,
} from './json-cfg-pose-adapter';
import {
  ReadableVirtualFileSystem,
  readStringToEnd,
  loadImageFromArrayBuffer,
  scaleImage,
  FileNotFoundError,
} from '../../..';
...
export async function loadJsonModelFromVFS(
  ...
  for (let i = 0; i < model3json.FileReferences.Textures.length; i++) {
    ...
    // load texture
    let imgDom: TexImageSource = await loadImageFromArrayBuffer(
      await vfs.read(src)
    );
    const maxTextureSize = gl.getParameter(gl.MAX_TEXTURE_SIZE);
    if (
      imgDom.width > gl.getParameter(gl.MAX_TEXTURE_SIZE) ||
      imgDom.height > gl.getParameter(gl.MAX_TEXTURE_SIZE)
    ) {
      imgDom = await scaleImage(
        imgDom,
        maxTextureSize /
          (imgDom.width > imgDom.height ? imgDom.width : imgDom.height)
      );
    }
  ...
  if (options?.loadMotions) {
    ...
    for (const item of vfs.tryFindAndReadAll!(findMotionFile)) {
      if (item !== undefined) {
        ...
        const clip = loadCubismMotionClipFromJson(
          cubismModel,
          motion3Json,
          motionName,
          idleNames.has(motionName)
        );
  ...
  if (options?.loadExpressions) {
    ...
    for (const [fileName, filePromises] of vfs.tryFindAndReadAll!(
      findExpFile
    )) {
      // @ts-ignore
  ...
  // #region UserData
  if (
    model3json.FileReferences.UserData !== undefined &&
