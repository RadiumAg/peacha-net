...
export class Live2dPreviewMinComponent implements AfterViewInit, OnDestroy {
  ...
  mouseMove(event: MouseEvent) {
    this.model.setParameterByIndex(
      ...
      ((event.clientX - (screen.width - 20 - 150)) / screen.width) * 30
    );
    this.model.setParameterByIndex(
      this.model.PARAM_ANGLE_Y,
      -((event.clientY - (screen.height - 200)) / screen.height) * 30
    );
    this.model.setParameterByIndex(
      this.model.PARAM_BODY_ANGLE_X,
      ((event.clientX - (screen.width - 20 - 150)) / screen.width) * 10
    );
    this.model.setParameterByIndex(
      this.model.PARAM_BODY_ANGLE_Y,
      -((event.clientY - (screen.height - 200)) / screen.height) * 10
    );
    this.model.setParameterByIndex(
      this.model.PARAM_EYE_BALL_X,
      (event.clientX - (screen.width - 20 - 150)) / screen.width
    );
    this.model.setParameterByIndex(
      this.model.PARAM_EYE_BALL_Y,
      -((event.clientY - (screen.height - 200)) / screen.height)
    );
  }
