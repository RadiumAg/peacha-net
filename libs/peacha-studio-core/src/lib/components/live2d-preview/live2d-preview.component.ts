...
export class Live2dPreviewComponent implements AfterViewInit {
  ...
  onMouseMove(event: MouseEvent): void {
    ...
    if (this.followMouse && !this.cs) {
      if (this.model && (event.target as any).localName === 'canvas') {
        this.blendshapes.set(
          this.model.PARAM_ANGLE_X,
          this.model.getParameterMaximumValueByIndex(this.model.PARAM_ANGLE_X) *
            offsetX
        );
        this.blendshapes.set(
          this.model.PARAM_ANGLE_Y,
          -this.model.getParameterMaximumValueByIndex(
            this.model.PARAM_ANGLE_Y
          ) * offsetY
        );
        this.blendshapes.set(
          this.model.PARAM_ANGLE_Z,
          this.model.getParameterMaximumValueByIndex(this.model.PARAM_ANGLE_Z) *
            offsetX *
            offsetY
        );
        this.blendshapes.set(
  ...
  async reloadModel(): Promise<void> {
    ...
    this.expressionAnimator = CubismExpressionComponent.oneFrom(
      this.entity
    )?.data;
  ...
  parameterValuesDoFade(): void {
    if (this.parameterValuesFadeStartTime) {
      ...
      const fadeDefault = getEasingSine(
        (this.fadeDefaultTime - timeFromStart) / this.fadeDefaultTime
      );
      if (fadeDefault <= 1 && fadeDefault > 0) {
        this.parameterValues.forEach((parameterValue, index) => {
          this.parameterValues.get(index)!.value =
            (this.parameterValues.get(index)!.value - parameterValue.default) *
              fadeDefault +
  ...
  tick(): void {
    ...
    this.parameterValues.forEach((parameterValue, index) => {
      if (this.blendshapes.has(parameterValue.index) && !parameterValue.lock) {
        this.parameterValues.get(index)!.value = this.blendshapes.get(
          parameterValue.index
        )!;
      }
      this.model.setParameterLockByIndex(parameterValue.index, false);
      this.model.setParameterByIndex(
        parameterValue.index,
        parameterValue.value
      );
      this.model.setParameterLockByIndex(
        parameterValue.index,
        parameterValue.lock
      );
    });
    ...
    this.parameterValues.forEach((parameterValue, index) => {
      this.parameterValues.get(index)!.value = this.model.getParameterByIndex(
        parameterValue.index
      );
    });
    this.partOpacities.forEach((partOpacity, index) => {
      this.partOpacities.get(index)!.value = this.model.getPartOpacityByIndex(
        partOpacity.index
      );
    });
  ...
  async ngAfterViewInit(): Promise<void> {
    if (typeof WheelEvent) {
      (this.elementRef.nativeElement as HTMLDivElement).addEventListener(
        'mousewheel',
  ...
  ngOnDestroy(): void {
    ...
    this.world = null;
    if (typeof WheelEvent) {
      (this.elementRef.nativeElement as HTMLDivElement).removeEventListener(
        'mousewheel',
        this.wheel.bind(this)
      );
    } else {
  ...
  async requestFullScreen(): Promise<void> {
    if (isDocumentInFullScreenMode()) {
      ...
      await (this.elementRef
        .nativeElement as HTMLDivElement).requestFullscreen();
    }
  }
  ...
  playMotion(index: number): void {
    this.motionAction = this.motionAnimator.start(
      this.motionAnimator.clips[index]
    );
  }
  ...
  playExpression(index: number): void {
    if (this.expressionActions.has(index)) {
      ...
      this.expressionActions.set(
        index,
        this.expressionAnimator.start(this.expressionAnimator.clips[index])
      );
    }
  }
}
